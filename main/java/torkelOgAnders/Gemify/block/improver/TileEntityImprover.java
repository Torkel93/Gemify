...
public class TileEntityImprover extends TileEntity implements ISidedInventory {
	...
	public String getInventoryName() {
		return this.hasCustomInventoryName() ? this.furnaceName : "Improver";
	}
	...
	public void updateEntity() {
		...
		if (flag != this.furnaceBurnTime > 0) {
			flag1 = true;
			Improver.updateBlockState(this.furnaceBurnTime > 0, this.worldObj, this.xCoord, this.yCoord, this.zCoord);
	}
	...
	private boolean canSmelt() {
		if (this.furnaceItemStacks[0] == null) {
			...
			ItemStack itemstack = RecipesImprover.smelting().getSmeltingResult(this.furnaceItemStacks[0]);
	...
	public int ejectItems(int slot, int x, int y, int z){
		...
		TileEntitySmelter machine = (TileEntitySmelter) worldObj.getTileEntity(x, y, z);
	...
	public boolean checkForMachine(int x, int y, int z){
		...
		if (block == Gemify.machineSmelter || block == Gemify.machineSmelterActive){
			return true;
		}
		return false;
}
