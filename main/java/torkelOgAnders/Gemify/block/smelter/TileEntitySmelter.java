...
public class TileEntitySmelter extends TileEntity implements ISidedInventory {
	...
	public String getInventoryName() {
		return this.hasCustomInventoryName() ? this.furnaceName : "Smelter";
	}
	...
	public void updateEntity() {
		...
		if (flag != this.furnaceBurnTime > 0) {
			flag1 = true;
			Smelter.updateBlockState(this.furnaceBurnTime > 0, this.worldObj, this.xCoord, this.yCoord, this.zCoord);
	}
	...
	public int ejectItems(int slot, int x, int y, int z){
		...
		TileEntityChest chest = (TileEntityChest) worldObj.getTileEntity(x, y, z);
		
		ItemStack outitemstack = this.getStackInSlot(slot);
		if (this.furnaceItemStacks[slot] != null){
			for (int i = 0; i <= chest.getSizeInventory() -1; i++){
				if (chest.getStackInSlot(i) == null) {
					chest.setInventorySlotContents(i, new ItemStack(outitemstack.getItem(), outitemstack.stackSize));
				...
				else if (outitemstack.getItem() == chest.getStackInSlot(i).getItem() && chest.getStackInSlot(i).stackSize <= 63 && this.furnaceItemStacks[slot] != null) {
						
					int hvormye = 64 - chest.getStackInSlot(i).stackSize;
					if (outitemstack.stackSize <= hvormye){
							hvormye = outitemstack.stackSize;
					}
					chest.getStackInSlot(i).stackSize += hvormye;
